...
class Storage {
  ...
  static ProfileModel? get user {
    final userString = _get<String>(_Keys.profile);
    return _parseUser(userString);
  }

  static Stream<ProfileModel?> get userStream {
    return _rxPrefs
        .getStringStream(_Keys.profile)
        .map((event) => _parseUser(event));
  }

  static ProfileModel? _parseUser(String? userString) {
    try {
      if (userString != null) {
        return ProfileModel.fromJson(json.decode(userString));
      }
    } catch (e) {
      logger.e(e);
    }
  ...
  static Future<void> setUser(ProfileModel? val) =>
      _set(_Keys.profile, json.encode(val?.toJson()));
